    ...
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <div className="sidebar-note">
          <p className="eyebrow inverse">Backend contract</p>
          <strong><span className="api-dot" />Owner session active</strong>
          <p>Semua aksi UI diarahkan ke REST API live. Tidak ada team, assignment, atau approval eksternal.</p>
        </div>
      </aside>
      ...
      <main className="main">
        <header className="topbar">
          <div className="topbar-left">
            ...
            <div className="topbar-caption">Device timezone / UTC data store</div>
          </div>
...
function DashboardScreen({ activeProjects, activeSeconds, activeTimer, dashboard, onNewTask, onOpenProject, onOpenTask, onStopTimer, projects, setScreen }: { activeProjects: Project[]; activeSeconds: number; activeTimer: DashboardReport['activeTimer']; dashboard: DashboardReport | null; onNewTask: () => void; onOpenProject: (project: Project) => void; onOpenTask: (task: Task) => void; onStopTimer: () => Promise<void>; projects: Project[]; setScreen: (screen: Screen) => void }) {
  ...
  return <section className="screen">
    <div className="screen-intro"><div><p className="eyebrow">Personal command center</p><h1>Make the work<br />visible.</h1><p>Mulai dari task yang sedang berjalan, simpan setiap sesi, lalu review sebelum pekerjaan ditutup.</p></div><div className="button-row"><button className="btn btn-outline" onClick={() => setScreen('reports')}>View reports -&gt;</button><button className="btn btn-primary" onClick={onNewTask}>+ New task</button></div></div>
    ...
    <div className="grid dashboard-grid" style={{ marginTop: 16 }}><article className="timer-card"><div className="timer-context"><p className="eyebrow">Global timer / one slot only</p><span className="revision-badge">{activeTimer ? revisionLabel(activeTimer.revisionNumber) : 'No active entry'}</span></div><h2 className="timer-task-name">{activeTimer ? activeTimer.task.title : 'Choose a task.'}</h2><div className="timer-project">{activeTimer ? `${activeTimer.task.project.name} / active task` : 'Timer tersedia pada task TO_DO atau IN_PROGRESS.'}</div><div className="timer-value">{formatDuration(activeSeconds, true)}</div><div className="button-row">{activeTimer ? <><button className="btn btn-primary" onClick={() => void onStopTimer()}>Stop &amp; save session</button><button className="btn btn-outline" onClick={() => onOpenTask(activeTimer.task)}>Open task</button></> : <button className="btn btn-primary" onClick={() => setScreen('projects')}>Browse tasks</button>}</div></article><aside className="side-note"><p className="eyebrow">The loop</p><h2>Every session leaves a trace.</h2><p>Timer berhenti hanya menyimpan Time Entry. Status task baru berubah saat kamu mengirimkannya untuk review.</p><div className="mini-flow"><span>work</span><i>-&gt;</i><span>review</span><i>-&gt;</i><span>done</span></div></aside></div>
...
function ModalHeader({ children, close, eyebrow, title }: { children: React.ReactNode; close: () => void; eyebrow: string; title: string }) {
  return <div className="modal-header"><div><p className="eyebrow">{eyebrow}</p><h2>{title}</h2><p>{children}</p></div><button className="close-modal" onClick={close} aria-label="Tutup">x</button></div>;
}
...
function ConfirmModal({ actionLabel, close, description, eyebrow, onAction, title }: { actionLabel: string; close: () => void; description: string; eyebrow: string; onAction: () => Promise<void>; title: string }) {
  return <><ModalHeader close={close} eyebrow={eyebrow} title={title}>{description}</ModalHeader><div className="modal-callout">Hard delete tidak tersedia agar data historis tetap aman.</div><div className="modal-actions"><button className="btn btn-quiet" onClick={close}>Cancel</button><button className="btn btn-danger" onClick={() => void onAction()}>{actionLabel}</button></div></>;
}
