...
type Modal =
  ...
  | { type: 'editEntry'; task: Task; entry: TimeEntry }
  | { type: 'voidEntry'; task: Task; entry: TimeEntry }
...
export default function Home() {
  ...
  const filteredTasks = projectTasks.filter((task) => taskFilter === 'ARCHIVED' ? !!task.archivedAt : !task.archivedAt && (taskFilter === 'ALL' || task.status === taskFilter));
    ...
    <div className="app-shell">
      ...
      {modal && <ModalView modal={modal} close={() => setModal(null)} projects={projects} submit={async (event) => {
        ...
        if (modal.type === 'editEntry') {
          ...
          const endedAt = String(form.get('endedAt') || '');
          await runAction(async () => {
            await api.updateTimeEntry(modal.entry.id, {
              startedAt: new Date(startedAt).toISOString(),
              endedAt: new Date(endedAt).toISOString(),
              revisionNumber: Number(form.get('revisionNumber')),
              description: String(form.get('description') || ''),
            });
            await refreshTask(modal.task.id);
          }, 'Time entry diperbarui.');
        }
        ...
        if (kind === 'restoreTask' && modal.type === 'archiveTask') await runAction(async () => { await api.updateTask(modal.task.id, { archived: false }); if (modal.task.projectId) await refreshProject(modal.task.projectId); setScreen('project-detail'); }, 'Task direstore.');
        if (kind === 'voidEntry' && modal.type === 'voidEntry') await runAction(async () => { await api.voidTimeEntry(modal.entry.id, 'Voided from frontend'); await refreshTask(modal.task.id); }, 'Time entry dibatalkan.');
...
function ModalView({ action, close, modal, projects, submit }: { action: (kind: string) => Promise<void>; close: () => void; modal: Exclude<Modal, null>; projects: Project[]; submit: (event: FormEvent<HTMLFormElement>) => Promise<void> }) {
  if (isModalType(modal, 'manualEntry')) return <TimeEntryModal close={close} submit={submit} task={(modal as { type: 'manualEntry'; task: Task }).task} />;
  if (modal.type === 'editEntry') return <TimeEntryModal close={close} submit={submit} task={modal.task} entry={modal.entry} />;
  if (modal.type === 'voidEntry') return <ConfirmModal close={close} title="Void time entry?" description="Entry tidak akan dihitung dalam reports, tetapi tetap tersimpan sebagai histori koreksi." actionLabel="Void entry" onAction={() => action('voidEntry')} />;
  if (modal.type === 'archiveTask' && modal.task.archivedAt) return <ConfirmModal close={close} title="Restore task?" description="Task akan kembali muncul di daftar active task dan bisa menerima timer atau manual entry." actionLabel="Restore task" onAction={() => action('restoreTask')} />;
...
function TimeEntryModal({ close, entry, submit, task }: { close: () => void; entry?: TimeEntry; submit: (event: FormEvent<HTMLFormElement>) => Promise<void>; task: Task }) {
  const [startedAt, setStartedAt] = useState(toDatetimeLocal(entry?.startedAt));
  const [endedAt, setEndedAt] = useState(toDatetimeLocal(entry?.endedAt));
  const start = startedAt ? new Date(startedAt) : null;
  const end = endedAt ? new Date(endedAt) : null;
  const seconds = start && end && end > start ? Math.floor((end.getTime() - start.getTime()) / 1000) : 0;
  const warnings = timeEntryWarnings(task, startedAt, endedAt, entry?.id);

  return <div className="modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}><div className="modal"><ModalHeader title={entry ? 'Edit time entry' : 'Add manual entry'} close={close}>{entry ? 'Perbaiki jam, revision, atau deskripsi entry.' : 'Catat sesi lama dengan tanggal dan jam custom.'}</ModalHeader><form className="modal-form" onSubmit={(event) => void submit(event)}><Field label="Started at"><input name="startedAt" type="datetime-local" required value={startedAt} onChange={(event) => setStartedAt(event.target.value)} /></Field><Field label="Ended at"><input name="endedAt" type="datetime-local" required value={endedAt} onChange={(event) => setEndedAt(event.target.value)} /></Field><Field label="Revision"><select name="revisionNumber" defaultValue={entry?.revisionNumber ?? task.currentRevision}>{Array.from({ length: task.currentRevision + 1 }, (_, revision) => <option value={revision} key={revision}>{revisionLabel(revision)}</option>)}</select></Field><Field label="Description"><textarea name="description" defaultValue={entry?.description || ''} placeholder="Contoh: Migrasi catatan waktu lama" /></Field>{startedAt && endedAt && <div className="entry-preview">Preview: {formatTimeOnly(startedAt)} - {formatTimeOnly(endedAt)} = {formatDuration(seconds)}</div>}{warnings.length > 0 && <div className="warning-list">{warnings.map((warning) => <div key={warning}>{warning}</div>)}</div>}<ModalActions close={close} submitLabel={entry ? 'Save changes' : 'Save entry'} /></form></div></div>;
}
...
function TaskRow({ active, onOpen, onRestore, onStart, onStop, task }: { active: boolean; onOpen: () => void; onRestore: () => void; onStart: () => void; onStop: () => Promise<void>; task: Task }) {
  const seconds = (task.timeEntries || []).filter((entry) => entry.status !== 'VOIDED').reduce((total, entry) => total + (entry.durationSeconds || 0), 0);
  return <div className="task-row"><button className="task-main text-link" onClick={onOpen}><span className={`task-status-mark ${statusMeta[task.status].className}`} /><span className="task-title"><strong>{task.title}</strong><small>{task.description || 'No description yet.'}</small></span></button><div className="task-actions"><span className="task-time">{formatDuration(seconds)}</span>{task.archivedAt ? <span className="status-badge archived">Archived</span> : <TaskStatusBadge status={task.status} />}{task.archivedAt ? <button className="btn btn-outline btn-sm" onClick={onRestore}>Restore</button> : active ? <button className="btn btn-acid btn-sm" onClick={() => void onStop()}>Stop</button> : canStart(task) ? <button className="btn btn-primary btn-sm" onClick={onStart}>Start</button> : <button className="btn btn-outline btn-sm" onClick={onOpen}>Review</button>}</div></div>;
}

function EntryRow({ entry, onEdit, onVoid }: { entry: TimeEntry; onEdit: () => void; onVoid: () => void }) {
  const voided = entry.status === 'VOIDED';
  return <div className={`entry-row ${voided ? 'voided' : ''}`}><span className={`entry-line ${entry.revisionNumber > 0 ? 'revision' : ''}`} /><div><strong>{formatDateTime(entry.startedAt)} - {formatDateTime(entry.endedAt)} {voided && <span className="status-badge archived">Voided</span>}</strong><small><span className="revision-badge">{revisionLabel(entry.revisionNumber)}</span> {entry.description || (voided ? entry.voidReason || 'Voided entry' : 'completed Time Entry')}</small></div><div className="entry-actions"><span className="entry-duration">{formatDuration(entry.durationSeconds || 0, true)}</span>{!voided && <><button className="btn btn-outline btn-sm" onClick={onEdit}>Edit</button><button className="btn btn-danger btn-sm" onClick={onVoid}>Void</button></>}</div></div>;
}
...
function timeEntryWarnings(task: Task, startedAt: string, endedAt: string, excludeId?: string) {
  if (!startedAt || !endedAt) return [];
  const start = new Date(startedAt);
  const end = new Date(endedAt);
  const warnings: string[] = [];
  if (end <= start) warnings.push('End time harus setelah start time.');
  const seconds = Math.floor((end.getTime() - start.getTime()) / 1000);
  if (seconds > 12 * 3600) warnings.push('Durasi lebih dari 12 jam. Cek kembali input waktunya.');
  if (start.toDateString() !== end.toDateString()) warnings.push('Entry melewati tengah malam. Report akan membagi durasi per hari.');
  if (start > new Date() || end > new Date()) warnings.push('Tanggal berada di masa depan.');
  const overlap = (task.timeEntries || []).some((entry) => {
    if (entry.id === excludeId || entry.status === 'VOIDED' || !entry.endedAt) return false;
    const existingStart = new Date(entry.startedAt);
    const existingEnd = new Date(entry.endedAt);
    return existingStart < end && existingEnd > start;
  });
  if (overlap) warnings.push('Waktu ini overlap dengan entry lain pada task yang sama.');
  return warnings;
}
...
function isModalType(modal: Exclude<Modal, null>, type: string) {
