...
export const api = {
  ...
  updateTimeEntry: (id: string, data: unknown) => apiRequest(`/time-entries/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
  voidTimeEntry: (id: string, reason?: string) => apiRequest(`/time-entries/${id}`, { method: 'DELETE', body: JSON.stringify({ reason }) }),
